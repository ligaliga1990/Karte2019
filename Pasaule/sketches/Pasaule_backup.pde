...
void setup() {
  ...
  println("Image width: " + world_map_img.width);
  println("Image height: " + world_map_img.height);
...
void get_dots() {
  for(int row = 0; row <= rows; row = row + 1)  { // // skaitlis = horizontālais daudzums
    for (int col = 0; col <= columns; col = col + 1) { // skaitlis ir vertikālais daudzums
      ...
      if(world_map_img.width < x || world_map_img.height - 30 < y) continue;

      int alpha = get_coord_alpha_value(world_map_img, x, y);
      if (alpha < 100) {
